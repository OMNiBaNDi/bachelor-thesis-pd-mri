#!/usr/bin/env python3
"""Copy SELECTED Forde NIfTI+JSON files into data/Forde/Fxxx/{BL,3Y,5Y}."""
...
LOG_PATH = Path("/nfs/br1_prosjekt/ParkWest/user/2026vae/AmundEspen/pd_thesis/scripts/forde_selection_log.csv")
DEST_ROOT = Path("/nfs/br1_prosjekt/ParkWest/user/2026vae/AmundEspen/pd_thesis/data/Forde")
...
def copy_pair(src: Path, dest_dir: Path, force: bool, dry_run: bool) -> None:
    ...
    if src_json.exists():
        ...
        print(f"WARN: JSON missing for {src}")
    ...
    with args.log_path.open() as f:
        ...
        for row in reader:
            ...
            if not selected_file:
                print(f"WARN: no selected file recorded for {dest_id} {tp}")
                skipped += 1
                continue
            ...
            if not src.exists():
                ...
                skipped += 1
                continue
            tp_dir = TIMEPOINT_DIRS[tp]
            dest_dir = args.dest_root / dest_id / tp_dir
