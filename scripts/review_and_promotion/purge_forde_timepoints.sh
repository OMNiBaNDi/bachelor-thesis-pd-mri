#!/bin/bash
# Remove all contents from data/Forde/Fxxx/{BL,3Y,5Y} for the curated patient list.
set -euo pipefail

DATA_ROOT=/nfs/br1_prosjekt/ParkWest/user/2026vae/AmundEspen/pd_thesis/data/Forde
...
usage() {
  cat <<'EOF'
Usage: purge_forde_timepoints.sh [--patients-file FILE]
Deletes everything under data/Forde/Fxxx/{BL,3Y,5Y} for the default 13 IDs.
  --patients-file FILE  Override the patient list (one ID per line, e.g., 02 or F02)
EOF
}
...
normalize() {
  ...
  token="${token^^}"
  if [[ "$token" =~ ^F([0-9]+)$ ]]; then
    printf 'F%03d' "$((10#${BASH_REMATCH[1]}))"
    return 0
  fi
  if [[ "$token" =~ ^([0-9]+)$ ]]; then
    printf 'F%03d' "$((10#$token))"
    return 0
  fi
  printf '%s' "$token"
}
